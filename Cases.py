...
TEST_CASES = {
    ...
    8: [("read", "alice", "emails.txt"), ("write", "alice", "emails.txt"), ("read", "alice", "username.txt"), ("read", "alice", "emails.txt")], # Alice reads emails.txt then writes to emails.txt, next she reads username.txt and writes to emails.txt
    9: [("read", "alice", "emails.txt"), ("read", "alice", "password.txt"), ("write")], # Alice reads emails.txt then writes to username.txt, next she reads password.txt and finally writes to password.txt
    ...
    12: [("read", "alice", "pub.txt"), ("read", "bob", "password.txt")], # Alice reads pub.txt then writes to password.txt, Bob then reads password.txt
    ...
    17: [("read", "alice", "username.txt"), ("set_level")], # Alice reads username.txt then sets her level to C (classified) and writes to emails.txt, Eve then reads emails.txt
